...
function createMockDb() {
  ...
  const selectGroupByMock = vi.fn().mockReturnValue({ orderBy: selectOrderByMock });
  const selectLeftJoinMock = vi.fn().mockReturnValue({
    where: vi.fn().mockReturnValue({
      groupBy: selectGroupByMock,
      orderBy: selectOrderByMock,
    }),
  });
  const selectInnerJoinMock = vi.fn().mockReturnValue({
    where: vi.fn().mockReturnValue({
      groupBy: selectGroupByMock,
      orderBy: selectOrderByMock,
    }),
