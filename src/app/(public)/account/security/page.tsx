    ...
    <div>
      ...
      <div className="rounded-lg border border-(--border-primary) p-6 mb-6">
        ...
        <form onSubmit={handleChangePassword} className="space-y-4 max-w-md">
          {success && <div className="text-sm text-(--color-success-500)">{success}</div>}
          ...
          <button type="submit" disabled={changePassword.isPending} className="py-2 px-4 rounded-lg font-medium text-sm bg-(--color-brand-500) text-white hover:bg-(--color-brand-600) transition-colors disabled:opacity-50">
      ...
      <div className="rounded-lg border border-(--border-primary) p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => revokeAll.mutate()} className="text-xs text-(--color-danger-500) hover:underline">
        ...
        <div className="space-y-3">
          {sessions?.map((s) => (
            <div key={s.id} className="flex items-center justify-between py-2 border-b border-(--border-primary) last:border-0">
              <div>
                <p className="text-sm">{s.userAgent?.slice(0, 50) ?? 'Unknown device'}</p>
                <p className="text-xs text-(--text-tertiary)">
                  {s.ipAddress ?? 'Unknown IP'} · {formatRelativeTime(s.createdAt)}
                </p>
              </div>
              <button onClick={() => revokeSession.mutate({ sessionId: s.id })} className="text-xs text-(--color-danger-500) hover:underline">
                Revoke
              </button>
            </div>
          ))}
